...
\begin{tikzpicture}
	...
	\begin{umlfragment}[type=loop, label=n\"achster Charakter]
		...
		\begin{umlfragment}[type=alt, label=Spieler ist am Zug, inner ysep=7.5]
			...
			\begin{umlfragment}[type=loop, label=n\"achste Operation, inner xsep = 7.5]
                \begin{umlcall}[op=RequestOperationMessage,dt=22.5]{Server}{Spieler}
				\end{umlcall}
                ...
                \begin{umlfragment}[type=opt, label=Fraktions\"anderung, inner ysep=5, inner xsep=7.5]
                    \begin{umlcall}[op=SpectatorRevealMessage, dt=10]{Server}{Zuschauer}
                    \end{umlcall}
                \end{umlfragment}
			\end{umlfragment}
